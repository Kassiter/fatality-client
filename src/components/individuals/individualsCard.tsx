...
class IndividualsCard extends React.Component<Props, State> {
  ...
  handleCheckedPress = () => {
    this.setState({
      ...
      amountMonth: !this.state.toggleChecked
        ? this.props.option_basic_cost
        : this.props.option_advanced_cost,
    });
  };
      ...
      return (
        <OverlayTrigger
          ...
          overlay={this.renderTooltip(
            this.props,
            'Необходимо войти, чтобы приобрести данный товар',
          )}
        >
          <div>
            <Button
              ...
              href={
                this.state.toggleChecked
                  ? this.props.option_basic_link
                  : this.props.option_advanced_link
              }
              className="btn-price"
            >
  ...
  render() {
    return (
      <div
        className="individuals-card"
        id="ind-card"
        style={{
          backgroundImage: `linear-gradient(rgba(0,0,0,0.4), rgba(0,0,0,0.4)), url(${this.props.img_url})`,
        }}
      >
        <h2>
          {this.props.name}
          {this.renderTooltipBase(this.props.tooltip)}
        </h2>
        {this.renderDiscount()}
